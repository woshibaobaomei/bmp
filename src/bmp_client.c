...
{
    bmp_client *A = (bmp_client*)(a - BMP_CLIENT_FD * sizeof(avl_node));
    bmp_client *B = (bmp_client*)(b - BMP_CLIENT_FD * sizeof(avl_node));
...
{
    bmp_client *A = (bmp_client*)(a - BMP_CLIENT_ADDR * sizeof(avl_node));
    bmp_client *B = (bmp_client*)(b - BMP_CLIENT_ADDR * sizeof(avl_node));
...
bmp_client_close(bmp_server *server, bmp_client *client, int reason)
{
    ...
    avl_remove(server->clients[BMP_CLIENT_FD], &client->avl[BMP_CLIENT_FD], NULL);
    avl_remove(server->clients[BMP_CLIENT_ADDR], &client->avl[BMP_CLIENT_ADDR], NULL);
...
bmp_client_process(bmp_server *server, int fd, int events)
{
    ...
    lookup.fd = fd;
    client = (bmp_client*) avl_lookup(server->clients[BMP_CLIENT_FD], &lookup.avl[BMP_CLIENT_FD], NULL);
...
bmp_client_create(bmp_server *server, int fd, struct sockaddr *addr, socklen_t slen)
{
    ...
    avl_insert(server->clients[BMP_CLIENT_FD], &client->avl[BMP_CLIENT_FD], NULL); 
    avl_insert(server->clients[BMP_CLIENT_ADDR], &client->avl[BMP_CLIENT_ADDR], NULL);
